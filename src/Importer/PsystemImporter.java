...
public class PsystemImporter 
{
	DatabaseAccess db;
	
	public PsystemImporter()
	{
	...
	{
		List<byte[]> myMultiset;
        myMultiset = db.multiRetrieveParcialMajor(level);
		for(byte[] aMultiset : myMultiset)
		{
    	    ByteArrayInputStream bi = new ByteArrayInputStream(aMultiset);
	...
	public void grabPsystem(String fileName)
	{
		try
		{
			...
			for(Membrane a : ps.getMembraneStructure().getAllMembranes())
			{
				...
				db.store( Arrays.asList("level0",a.getLabel()) , uuid, bo2.toByteArray());
			}
